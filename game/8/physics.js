// physics.js — Ball movement, collision, rendering
// Part of 8-Ball Pool. Exposes window.Pool shared namespace.
...
(() => {
    ...
    function applyReset(fullReset) {
        ...
        state.turn = state.nextTurn || 'player';
        state.nextTurn = null;
        state.aiState = state.turn === 'ai' ? 'thinking' : 'idle';
        state.aiShot = null;
        setMessage(state.turn === 'ai' ? 'Ход Снеговика. Разбивайте!' : 'Ход Игрока. Разбивайте!');
    }
    ...
    function update() {
        ...
        if (state.moving && !moving) { state.moving = false; if (Pool.onShotEnd) Pool.onShotEnd(); }
    }
    // Cinematic updates
    // -----------------------------------------------------------------------
    ...
    // ── Exports for rules.js ───────────────────────────────────────────
    Pool.state        = state;
    Pool.F            = F;
    Pool.BALL_R       = BALL_R;
    Pool.MAX_POWER    = MAX_POWER;
    Pool.CHARGE_RATE  = CHARGE_RATE;
    Pool.POCKETS      = POCKETS;
    Pool.FX2          = FX2;
    Pool.FY2          = FY2;
    Pool.BALL_COLORS  = BALL_COLORS;
    Pool.Ball         = Ball;
    Pool.isSolid      = isSolid;
    Pool.isStripe     = isStripe;
    Pool.isEightBall  = isEightBall;
    Pool.setMessage   = setMessage;
    Pool.resetGame    = resetGame;
    Pool.updateIntro  = updateIntro;
    Pool.update       = update;
    Pool.canvas       = canvas;
    Pool.W            = W;
    Pool.H            = H;
    Pool.draw         = draw;
    Pool.drawBallAt   = drawBallAt;
    Pool.initAudio         = initAudio;
    Pool.resumeAudio       = resumeAudio;
    Pool.soundCharge       = soundCharge;
    Pool.soundArrival      = soundArrival;
    Pool.soundShot         = soundShot;
    Pool.soundFoul         = soundFoul;
    Pool.soundWin          = soundWin;
    Pool.getBallsTouchedRail = () => ballsTouchedRail;
    Pool.resetBallsTouchedRail = () => { ballsTouchedRail = new Set(); };
    Pool.shoot             = null;  // set by rules.js
    Pool.onShotEnd    = null;  // set by rules.js
    Pool.updateHUD    = null;  // set by rules.js
