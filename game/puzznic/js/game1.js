// ================================================================
//  Puzznic JS — ИСПРАВЛЕННАЯ ВЕРСИЯ (гравитация и платформы работают)
// ================================================================
...
// ========== НАБОРЫ ТЕМ (встроенные прямо в файл) ==========
const THEMES = {
    animals: {
        name: "🐾 Животные",
    ...
    fruits: {
        name: "🍎 Фрукты",
        emojis: { '-':'🧱','~':'↔️','|':'↕️','E':'🍎','G':'🍐','B':'🍊','P':'🍋','X':'🍌','C':'🍉','D':'🍇','T':'🍒' },
        colors: { '~':'#ff9800','|':'#ff5722','E':'#e53935','G':'#8bc34a','B':'#ff9800','P':'#ffee58','X':'#fdd835','C':'#4caf50','D':'#9c27b0','T':'#e91e63' }
    },
    space: {
        name: "🚀 Космос",
        emojis: { '-':'🧱','~':'↔️','|':'↕️','E':'☀️','G':'🌙','B':'⭐','P':'🪐','X':'🌍','C':'🌕','D':'☄️','T':'👽' },
        colors: { '~':'#ff9800','|':'#ff5722','E':'#ffd700','G':'#e0e0e0','B':'#fdd835','P':'#ce93d8','X':'#64b5f6','C':'#fff9c4','D':'#ff8a65','T':'#b39ddb' }
    },
    food: {
        name: "🍕 Еда",
        emojis: { '-':'🧱','~':'↔️','|':'↕️','E':'🍕','G':'🍔','B':'🌮','P':'🥗','X':'🍣','C':'🍜','D':'🍦','T':'☕' },
    ...
    classic: {
        name: "👾 Классика",
        emojis: { '-':'🧱','~':'↔️','|':'↕️','E':'🔴','G':'🟠','B':'🟡','P':'🟢','X':'🔵','C':'🟣','D':'⚪','T':'⚫' },
        colors: { '~':'#ff9800','|':'#ff5722','E':'#e53935','G':'#ff9800','B':'#fdd835','P':'#4caf50','X':'#2196f3','C':'#9c27b0','D':'#e0e0e0','T':'#424242' }
    }
};
...
class PuzznicGame {
    ...
    updateBlockCounters() {
        ...
        for (const id of allIds) {
            ...
            opt.value = id;
            opt.textContent = id;  // ← только номер уровня (например, "1-1")
            sel.appendChild(opt);
        }
    }
    ...
    loadLevelFromSelect() {
        this.score = 0;
        this.loadLevel(document.getElementById('levelSelect').value);
        this.updateUI();
    }
    
    nextLevel() {
        const match = this.levelId.match(/(\d+)-(\d+)/);
        if (!match) return;
        const next = `${match[1]}-${parseInt(match[2]) + 1}`;
        if (getLevelData(next)) {
            this.loadLevel(next);
        } else {
