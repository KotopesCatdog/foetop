...
async function initGame() {
    ...
    // Загружаем боковую панель с рекордами
  //  await updateSidebarScores();
}
...
function resetBallSpeed() {
    ...
    console.log(`🔄 Скорость мяча сброшена до базовой`);
}
...
function initMovingBricks() {
    ...
    if (movingBricksList.length > 0) {
        console.log(`🔷 Найдено движущихся блоков: ${movingBricksList.length}`);
    }
}
...
function updateSpeedBar() {
    ...
    // Минимальное отображаемое значение 1.0, максимальное 3.0
    const MIN_DISPLAY = 1.0;
    const MAX_DISPLAY = 3.0;
...
function startSpeedIncreaseTimer() {
    ...
    speedIncreaseInterval = setInterval(() => {
        if (ballLaunched && !paused && !gameOver) {
            ...
            console.log(`⚡ Множитель: ${currentBallSpeedMultiplier.toFixed(1)}x`);
        }
    }, 20000);
}
...
function updateBallMovement() {
    ...
    if (ball.x < -100 || ball.x > W + 100 || ball.y < -100 || ball.y > H + 100) {
        ...
        ball.dy = -BASE_BALL_SPEED_Y;
        ballLaunched = false;
        console.log("⚠️ Мяч улетел, возвращаем");
        return;
    }
...
function activateBomb() {
    ...
    console.log("💣 activateBomb вызвана, bombCount:", bombCount);
    ...
    console.log("💣 Вызываем playExplosionSound()");
    playExplosionSound();
...
function loadNextLevel() {
    if (isRandomMode || currentLevel === "random") {
        ...
        console.log("🎲 Новый рандомный уровень:", newLevel.name);
    } 
...
function showHelpModal() {
    ...
    if (helpText) {
        helpText.innerHTML = `
            ...
            <p>• <kbd>J</kbd> - Старт - стоп музыки</p>
            ...
            <p>• <kbd>D</kbd> / <kbd>D</kbd> - Демо-режим</p>
            <hr>
       
            <p>⚡ <strong>Скорость мяча</strong> увеличивается каждые 20 сек.  </p>
            <p>❤️ <strong>+1 жизнь</strong> за каждые 1000 очков</p>
            <p>❤️ <strong>Бессмертие</strong> - клик на седечко</p>
